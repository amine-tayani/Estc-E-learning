...
<head>
  ...
  <title>Modifier Td | {{$td->titre}}</title>
      ...
      <div class="container-fluid">
        <div class="card shadow mb-4">
          <div class="card-header py-3">
            <div class="m-0 font-weight-bold text-primary"> <br> &nbsp; Modifier le td du {{$td->titre}} <br>

            </div> <br>
          </div>
          <div class="card-body">
            <form method="post" action="{{ route('td.update',$td->id) }}">
              @csrf
              @method('PUT')
              <div class=" form-group">
                <label style="color: #000; font-size: 15px;"> &nbsp; Titre</label>
                <input type="text" name="titre" class="form-control" placeholder="Enter le titre du td" required
                  value="{{ $td->titre }}">
              </div>
              <div class="form-group">
                <label style="color: #000; font-size: 15px;">&nbsp; Description</label>
                <input type="text" name="description" value="{{ $td->contenu }}" class="form-control"
              ...
              <div class="form-group">
                <label style="color: #000; font-size: 15px;"> &nbsp; Importer td</label>
                <input type="file" value="{{ $td->pdf }}" name="file" id="file" class="form-control">
              </div>
